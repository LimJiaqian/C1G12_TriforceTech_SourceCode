...
export default function SmartPrediction({ myUser }) {
  const [analysis, setAnalysis] = useState(null); // start as null
  ...
  useEffect(() => {
    async function fetchData() {
      if (!myUser) return;
      try {
        const res = await fetch(`http://127.0.0.1:5000/user/${myUser.user_id}/ai-analysis`);
  ...
  return (
    <div className="w-full bg-[#F4EEFF] rounded-xl p-6 shadow-md">
      ...
      <p className="mt-1 text-gray-700">
        Save <span className="font-semibold text-[#6C00FF]">{analysis.savedKwh} kWh</span> next month to climb the leaderboard!
      </p>
      ...
      <div className="mt-5 bg-[#EAE2FF] rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-[#E2D9FF] rounded-xl p-4">
            ...
            <p className="text-sm text-gray-600 mt-1">
              If rank #{myUser.Rank - 1} also donates ~{analysis.savedKwh} kWh more
            </p>
          </div>
        ...
        <div className="mt-4 bg-[#EFE8FF] rounded-xl p-4 flex items-start gap-2 text-gray-700">
          ...
          <p className="text-sm">
            Pro Tip: Aim for <strong>{analysis.maxNeeded} kWh</strong> to secure your spot — top donors often increase contributions near month-end!
          </p>
        </div>
