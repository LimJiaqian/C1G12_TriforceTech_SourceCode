...
def query_jamai_chat(query_text: str, table_id: str = "Chatbox") -> dict:
    ...
    try:
        ...
        if response_text.strip() == "FALLBACK_MODE":
            print(f"FALLBACK_MODE detected - switching to Google Gemini")
            try:
                # Use Gemini as fallback - returns ONLY the text string
                ...
                return {
                    "success": True,
                    "response": final_answer,  # Contains only the text string
                    "table_id": table_id,
                    "fallback_used": True,
