...
def process_enquiry(input_data: str, input_type: str = 'text') -> dict:
    ...
    try:
        if input_type == 'audio_path':
            ...
            # Step 3: Query JamAI chat with the transcribed text
            print(f"Querying JamAI with transcript...")
            chat_response = query_jamai_chat(transcript_text)
            ...
            return {
                ...
                "chat_success": chat_response.get("success", False)
            }
        ...
        if response.rows:
            ...
            if target_col:
                ...
                if hasattr(target_col, 'model_dump'):
                    print(f"[DEBUG] Found Pydantic v2 model - using model_dump()")
                    target_col = target_col.model_dump()
                    print(f"[DEBUG] Converted to dict, keys: {list(target_col.keys())[:10]}")
                elif hasattr(target_col, 'dict'):
                    print(f"[DEBUG] Found Pydantic v1 model - using dict()")
                    target_col = target_col.dict()
                    print(f"[DEBUG] Converted to dict, keys: {list(target_col.keys())[:10]}")
                ...
                if isinstance(target_col, dict):
                    ...
                    if 'choices' in target_col and len(target_col['choices']) > 0:
                        try:
                            ...
                            print(f"[DEBUG] Extracted from OpenAI dict: {response_text[:100]}...")
                        ...
                        print(f"[DEBUG] Extracted from dict fallback: {response_text[:100]}...")
                
                # Try OpenAI-style object structure (has .choices attribute)
                elif hasattr(target_col, 'choices') and hasattr(target_col.choices, '__iter__'):
                    # OpenAI-style: object.choices[0].message.content
                    try:
                        content = target_col.choices[0].message.content
                        response_text = str(content)
                        print(f"[DEBUG] Extracted from OpenAI object: {response_text[:100]}...")
                    except (IndexError, AttributeError) as e:
                        print(f"[DEBUG] OpenAI object parse error: {e}")
                ...
                # If it's a string already (check for stringified object)
                    ...
                    # Check if it's a stringified object (starts with class name or contains metadata)
                    if target_col.startswith("ChatCompletion") or "choices=" in target_col or "content='" in target_col:
                        print(f"[DEBUG] ⚠️ Detected stringified object - attempting regex extraction")
                        # Try to extract content using regex
                        import re
                        match = re.search(r"content='(.*?)'", target_col, re.DOTALL)
                        if match:
                            response_text = match.group(1)
                            print(f"[DEBUG] ✅ Extracted via regex: {response_text[:100]}...")
                        else:
                            print(f"[DEBUG] ❌ Regex extraction failed")
                            response_text = "Error: Could not extract content from stringified object."
                    else:
