    ...
    try:
        ...
        if response.rows:
            ...
            if target_col:
                ...
                print(f"[DEBUG] target_col value: {target_col}")
                
                # Try to get the text value from different possible structures
                if isinstance(target_col, dict):
                    # If it's a dict, get the 'text' or 'value' field
                    response_text = str(target_col.get("text") or target_col.get("value") or "")
                    ...
                    # Check if it's an object with a string representation that's not useful
                    # Try to access common OpenAI response patterns
                    if hasattr(target_col, 'choices') and len(target_col.choices) > 0:
                        # OpenAI-style response object
                        response_text = str(target_col.choices[0].message.content)
                    elif hasattr(target_col, '__dict__'):
                        # It's an object - check its attributes
                        obj_dict = target_col.__dict__
                        print(f"[DEBUG] Object attributes: {obj_dict.keys()}")
                        # Try to find content in common fields
                        response_text = (
                            obj_dict.get('text') or 
                            obj_dict.get('content') or 
                            obj_dict.get('value') or
                            "Error: Unable to extract response text from object."
                        )
                    else:
                        response_text = "Error: Unable to extract response text."
            else:
